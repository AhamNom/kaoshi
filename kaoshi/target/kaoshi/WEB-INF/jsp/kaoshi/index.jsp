...
<body>
    <blockquote class="layui-elem-quote quoteBox">
            <a class="layui-btn layui-btn-normal add">发布试卷</a>
    </blockquote>
...
layui.use(['form','layer','table','laytpl','util'],function(){
    ...
    var tableIns = table.render({
        elem: '#List',
        url : 'http://localhost:8080/kaoshi/admin/kaoshi/getList',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limit : 20,
    ...
    function add(){
        var index = layui.layer.open({
            title : "发起试卷",
            type : 2,
            content : 'http://localhost:8080/kaoshi/admin/kaoshi/add',
    ...
    function edit(data){
        var index = layui.layer.open({
            title : "修改",
            type : 2,
            content : "http://localhost:8080/kaoshi/admin/kaoshi/edit?id="+data.id,
            success : function(layero, index){
    ...
    table.on('tool(List)', function(obj){
        ...
        if (Event === 'del') {
            layer.confirm('您确定要删除这条信息？',{icon:3,title:'提示信息'},function(index){
                $.get("http://localhost:8080/kaoshi/admin/kaoshi/del",{
                    id : Data.id,
                },function(data){
                    if (data.status!=200) {
                        layer.msg(data.info);
